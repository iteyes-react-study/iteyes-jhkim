...
```
앞에서부터 조건식, 물음표(?), 참일 경우 실행하는 로직,:(콜론), 조건문이 거짓일 경우 실행할 표현식이 문법
기본예제를 직접 만들어 사용해보면 크게 어렵지 않다는걸 느낄 수 있음
<br>
</br>
...
```
위 코드에서 ```if/else``` 구문을 삼항연산자를 활용하면 위처럼 한줄에 처리할 수 있음, 하지만 많은 중첩이 필요한 코드라면 삼항 연산자를 고려해볼 필요가 있음
```javascript
// 삼항연산자 사용 중첩코드
...
이번 내용부터는 자바스크립트의 코드를 처리하는 테크닉이 아닌
*자바스크립트가 동작하는 방식*에 대한 내용으로 가장 중요한 파트 중 하나라 생각하며 확실히 이해하기 위해 상세히 작성하겠음

**시작하기 전에**

동기 비동기를 다루기전에 알아야 할 `프로세스`와 `스레드`기본 개념에 대해 보겠습니다.
...
여기서 *브라우저*는(프로세스) 이고 *인터넷 강의 스트리밍*(스레드1)과 *유뷰트 영상 재생*(스레드2)라고 생각할 수 있음
```
위 개념을 프로세스와 스레드라 부른다. 혹시 자바의 객체 개념을 알고 있는 사람이라면 `프로세르를 "프로그램의 인스턴스"`, 스레드를 `인스턴스 안에서 "실행되는 하나의 단위"`라는 
...
위 예시에서는 하나의 프로세스에서 2개의 쓰레드가 동작했고 이러한 개념을 `멀티 쓰레드`라고 한다.<br> 하지만 자바스크립트는 한번에 하나의 일만 수행하는 `싱글 스레드` 언어이다.
...
웹사이트에서 버튼 하나에 ~~30초~~-> 3초 이상 응답이 없다면 만약 나였으면 새로고침을 눌렀거나 뒤로가기를 눌렀을 것이다.
...
```
`Promise` 객체를 사용함으로써 얻을 수 있는 장점은 다음과 같다.
