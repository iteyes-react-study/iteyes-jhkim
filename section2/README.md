...
**정리**
- 자바스크립트는 하나의 `Call Stack`을 가지고 있는 `싱글 스레드 언어` 이고 `콜백 함수`를 활용해 `동기 방식`의 단점을 보완한 비동기 방식 개발을 할 수 있음
- *동기 방식*, *비동기 방식*의 개념을 알고 있으면 앞으로 개발을 하는데 있어 큰 도움이 될 것이라 생각함
...
`await`를 적용해 동기로 요청을 받은 응답 데이터 그림
